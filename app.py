...
import numpy as np
from skin_cancer_detection import output
...
def show():
    ...
    d=np.array(['Actinic keratoses','Basal cell carcinoma','Benign keratosis-like lesions','Dermatofibroma','Melanocytic nevi','Melanoma','Vascular lesions'])
    ...
    class_ind=np.argmax(val,axis=1)
    

    result=str(d[class_ind])[2:-2]
